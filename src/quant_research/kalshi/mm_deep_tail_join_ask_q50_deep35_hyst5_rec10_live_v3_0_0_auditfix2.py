...
"""Dashboard-compatible V3.0.0 deployment shim for Deep35 identity AUDITFIX2.

Public V3.0.0 / V1.13 version strings are intentionally preserved so the existing
notebook dashboard continues to work unchanged.  Exact source commit plus the
patch-version fields distinguish this deployment from prior defective runs.
...
from . import mm_deep_tail_join_ask_live_v1_13_2_identity_reconcile as LIVE


DEPLOY_VERSION = V300.DEPLOY_VERSION
PATCH_VERSION = "V3_0_0_AUDITFIX2"
MODULE_NAME = (
    "quant_research.kalshi."
    "mm_deep_tail_join_ask_q50_deep35_hyst5_rec10_live_v3_0_0_auditfix2"
)

Q50_ARM = "LIVE_DEEP35_HYST5_Q50_REC10_12H_V300_AUDITFIX2"
...
SUPERVISOR_LOG_FILE = "supervisor_v3_0_0_deep35_auditfix2.log"
GUARDIAN_LOG_FILE = "guardian_v3_0_0_deep35_auditfix2.log"
STARTUP_TIMEOUT_S = V300.STARTUP_TIMEOUT_S


def _install_patch():
    """Rebind the audited V3.0.0 operational wrapper to V1.13.2 identity safety."""
    V300.LIVE = LIVE
    ...
    # Detached children must import this shim and use the fixed child engine.
...
def static_self_check(*, show=True):
    ...
    checks = {
        ...
        "identityfix_live_static_ok": live.get("ok") is True,
        ...
        "deploy_patch_version_exact": PATCH_VERSION == "V3_0_0_AUDITFIX2",
        "engine_patch_version_exact": getattr(LIVE, "PATCH_VERSION", None)
        == "DEEP35_IDENTITY_RECONCILE_V1_13_2",
        "detached_module_is_auditfix2": RUNTIME.MODULE_NAME == MODULE_NAME,
        "runtime_live_is_identityfix": RUNTIME.LIVE is LIVE,
        "parent_live_is_identityfix": P.LIVE is LIVE,
        "guardian_live_is_identityfix": V2963.LIVE is LIVE,
    ...
    if show:
        print("=" * 176)
        print("V3.0.0 DEEP35 AUDITFIX2 STATIC CHECK — NO API / NO ORDERS")
        print("=" * 176)
        for k, v in out.items():
            print(f"{k:108s}: {v}")
    if not ok:
        raise RuntimeError(f"V3.0.0 AUDITFIX2 static self-check failed: {out}")
    return out
