...
"""V2.9.8.6 Q50 M1->M12 guard 12h deployment.

Diagnostic-only operational delta from V2.9.8.5:
- keep trader-process-group RSS warning at 4096 MiB;
- raise the hard fail-closed trader-process-group RSS limit from 6144 MiB to
  8192 MiB so the next run can measure whether the observed ~6.36 GiB spike is
  bounded/transient or continues materially higher.

The frozen Q50 M1->M12 strategy, V1.12.2 cancel/REST reconciliation, $20 session
loss trigger, $125 minimum starting equity, recorder, M12 cleanup, and fresh
trader-process rotation are unchanged.  Importing this module performs no API
calls and sends no orders.
"""

from . import mm_deep_tail_join_ask_q50_m12_guard_live_v2_9_8_5_12h_rotation as BASE


DEPLOY_VERSION = "MM_DEEP_TAIL_JOIN_ASK_Q50_M1_M12_GUARD_RECORD_M12_V2_9_8_6_12H_ROTATION"
MODULE_NAME = "quant_research.kalshi.mm_deep_tail_join_ask_q50_m12_guard_live_v2_9_8_6_12h_rotation"
Q50_ARM = "LIVE_DEEP_TAIL_Q50_M1_M12_GUARD_12H_V2986_ROTATION"
...
GENERATION_RSS_HARD_LIMIT_MB = 8192.0


def _install_patch():
    """Install audited V2.9.8.5 stack, then change only the RSS hard ceiling."""
    BASE._install_patch()
...
def static_self_check(*, show=True):
    ...
    checks = {
        "base_v2985_ok": base.get("ok") is True,
        ...
        "detached_module_points_to_v2986": RUNTIME.MODULE_NAME == MODULE_NAME,
        ...
        "trader_rss_hard_8192": GENERATION_RSS_HARD_LIMIT_MB == 8192.0,
        "runtime_warning_4096": RUNTIME.GENERATION_RSS_WARNING_MB == 4096.0,
        "runtime_hard_8192": RUNTIME.GENERATION_RSS_HARD_LIMIT_MB == 8192.0,
        "parent_warning_4096": P.GENERATION_RSS_WARNING_MB == 4096.0,
        "parent_hard_8192": P.GENERATION_RSS_HARD_LIMIT_MB == 8192.0,
        "guardian_warning_4096": V2963.GENERATION_RSS_WARNING_MB == 4096.0,
        "guardian_hard_8192": V2963.GENERATION_RSS_HARD_LIMIT_MB == 8192.0,
    ...
    if show:
        print("=" * 144)
        print("V2.9.8.6 Q50 M1->M12 8-GIB RSS DIAGNOSTIC STATIC CHECK — NO API / NO ORDERS")
        print("=" * 144)
        for k, v in out.items():
            print(f"{k:84s}: {v}")

    if not ok:
        raise RuntimeError(f"V2.9.8.6 static self-check failed: {out}")
    return out
