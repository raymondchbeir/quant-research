...
"""V2.9.9.1 real-money Q50 / 12h REC25 deployment.

Operational baseline is V2.9.9.0.  This wrapper swaps only the live engine to
V1.12.4, which fixes:
- REC25 trigger/exit atomicity: one certified BBO is both the trigger test and the
  frozen fixed passive exit price;
- exact Kalshi dollar equity semantics (``balance_dollars`` preferred);
- M12 exposure-first cleanup and authoritative zero-position verification before a
  ticker is marked finalized.

Entry, Q50, M1, M12 danger guard, REC25 anchor/threshold, no-reprice/no-chase,
12-hour parent runtime, $20 software loss trigger, $125 minimum equity, recorder,
rotation architecture, stale-REST reconciliation and orphan fail-closed behavior
remain unchanged.

Importing this module performs no API calls and sends no orders.
"""

from . import mm_deep_tail_join_ask_q50_m12_guard_rec25_live_v2_9_9_0_12h_rotation as BASE
from . import mm_deep_tail_join_ask_live_v1_12_4_rec25_atomic_exact_equity as REC25


DEPLOY_VERSION = "MM_DEEP_TAIL_JOIN_ASK_Q50_M1_M12_GUARD_REC25_V2_9_9_1_12H_ROTATION"
MODULE_NAME = "quant_research.kalshi.mm_deep_tail_join_ask_q50_m12_guard_rec25_live_v2_9_9_1_12h_rotation"
Q50_ARM = "LIVE_DEEP_TAIL_Q50_M1_M12_GUARD_REC25_12H_V2991"
...
def _install_patch():
    """Install V2.9.9.0 operations, then replace LIVE with corrected V1.12.4."""
    BASE._install_patch()
...
def static_self_check(*, show=True):
    ...
    checks = {
        "base_v2990_ok": base.get("ok") is True,
        "rec25_v1_12_4_ok": rec.get("ok") is True,
        "runtime_live_is_v1_12_4": RUNTIME.LIVE is REC25,
        "parent_live_is_v1_12_4": P.LIVE is REC25,
        "guardian_live_is_v1_12_4": V2963.LIVE is REC25,
        "rec25_engine_exact": LIVE.M12GuardRotatingGenerationEngine is LIVE.Rec25AtomicM12Engine,
        ...
        "detached_module_points_to_v2991": RUNTIME.MODULE_NAME == MODULE_NAME,
    ...
    if show:
        print("=" * 156)
        print("V2.9.9.1 Q50 M1->M12 GUARD + REC25 ATOMIC 12H STATIC CHECK — NO API / NO ORDERS")
        print("=" * 156)
        for k, v in out.items():
            print(f"{k:92s}: {v}")

    if not ok:
        raise RuntimeError(f"V2.9.9.1 static self-check failed: {out}")
    return out


def q50_preflight(*, show=True):
    """Read-only preflight using the corrected exact-dollar equity parser."""
...
def start_q50_12h_smoke(*, arm_phrase=None):
    """REAL-MONEY Q50 / 12h corrected REC25 entrypoint."""
    _install_patch()
