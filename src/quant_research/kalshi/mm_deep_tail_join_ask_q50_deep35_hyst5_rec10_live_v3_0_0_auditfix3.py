...
"""Dashboard-compatible V3.0.0 deployment shim for Deep35 AUDITFIX3.
...
version fields distinguish this deployment from prior defective runs.

AUDITFIX3 keeps all AUDITFIX1/AUDITFIX2 identity protections and changes only one
operational edge case: a stable-key ENTRY replacement waits non-fatally while the
prior create/cancel future is still pending instead of shutting down.
...
from . import mm_deep_tail_join_ask_live_v1_13_3_identity_lifecycle_wait as LIVE


DEPLOY_VERSION = V300.DEPLOY_VERSION
PATCH_VERSION = "V3_0_0_AUDITFIX3"
MODULE_NAME = (
    "quant_research.kalshi."
    "mm_deep_tail_join_ask_q50_deep35_hyst5_rec10_live_v3_0_0_auditfix3"
)

Q50_ARM = "LIVE_DEEP35_HYST5_Q50_REC10_12H_V300_AUDITFIX3"
...
# Dashboard/backward-compatible aliases.
Q100_Q = Q50_Q
...
SUPERVISOR_LOG_FILE = "supervisor_v3_0_0_deep35_auditfix3.log"
GUARDIAN_LOG_FILE = "guardian_v3_0_0_deep35_auditfix3.log"
STARTUP_TIMEOUT_S = V300.STARTUP_TIMEOUT_S


def _install_patch():
    """Rebind V3.0.0 operations to V1.13.3 lifecycle-wait identity safety."""
    V300.LIVE = LIVE
    ...
    # Detached subprocesses must import this exact shim and fixed child engine.
...
def static_self_check(*, show=True):
    ...
    checks = {
        ...
        "identity_lifecycle_wait_static_ok": live.get("ok") is True,
        ...
        "deploy_patch_version_exact": PATCH_VERSION == "V3_0_0_AUDITFIX3",
        "engine_patch_version_exact": getattr(LIVE, "PATCH_VERSION", None)
        == "DEEP35_IDENTITY_LIFECYCLE_WAIT_V1_13_3",
        "detached_module_is_auditfix3": RUNTIME.MODULE_NAME == MODULE_NAME,
        "runtime_live_is_v133": RUNTIME.LIVE is LIVE,
        "parent_live_is_v133": P.LIVE is LIVE,
        "guardian_live_is_v133": V2963.LIVE is LIVE,
    ...
    if show:
        print("=" * 176)
        print("V3.0.0 DEEP35 AUDITFIX3 STATIC CHECK — NO API / NO ORDERS")
        print("=" * 176)
        for k, v in out.items():
            print(f"{k:112s}: {v}")

    if not ok:
        raise RuntimeError(f"V3.0.0 AUDITFIX3 static self-check failed: {out}")
