...
"""Q50 one-hour real-money M1->M12 deep-tail forward validation.

This launcher is the full-window successor to Q50 V2.9.1.  It keeps Q50, the
one-hour validation horizon, the $20 start-to-current equity software stop, the
V1.7 long-run memory hardening, authenticated discovery, the external 2 GiB RSS
guardian, account audit and no-auto-scale policy.

The strategy-window change is explicit and isolated:
- M1 (60s): post YES/NO 5c passive entries.
- First fill selects the tail and cancels the opposite side.
- Selected entry may continue filling toward Q50 through M12.
- Once full Q50 is observed, post one fixed passive JOIN_ASK exit; no repricing.
- M12 (720s): persistent verified cleanup cancels all strategy resting orders and
  reduce-only IOC flattens residual inventory.
- Raw public data are recorded from M0 through M12 plus a 30-second label tail.

This remains a fresh forward validation, not proof of alpha.  The $20 software
stop is not a guaranteed realized-loss cap. Importing this module sends no orders.
"""
...
from . import mm_deep_tail_join_ask_live_v1_8_m1_m12 as LIVE
from . import mm_event_time_m0_m12_recorder_v6_auth as REC

DEPLOY_VERSION = "MM_DEEP_TAIL_JOIN_ASK_Q50_M1_M12_LIVE_V2_9_2_MEMORY_SAFE"
CORE = V282.CORE
Q50_ARM = "LIVE_DEEP_TAIL_Q50_M1_M12_1H_V292"
...
def static_self_check(*, show=True):
    ...
    checks = {
        "live_v1_8_m1_m12_ok": live.get("ok") is True,
        "m1_fixed_60s": abs(LIVE.M1_S - M1_S) < 1e-12,
        "m12_terminal_fixed_720s": abs(LIVE.TERMINAL_M12_S - M12_S) < 1e-12,
        "recorder_m0_m12": abs(REC.TRADE_WINDOW_START_S - 0.0) < 1e-12 and abs(REC.TRADE_WINDOW_END_S - M12_S) < 1e-12,
    ...
    out = {
        ...
        "legacy_cleanup_event_names": "M5_* labels map to actual M12/720s boundary in V1.8",
        **checks,
        "ok": bool(ok),
        "orders_sent": False,
    }
    if show:
        print("=" * 118)
        print("DEEP-TAIL Q50 M1->M12 V2.9.2 STATIC CHECK — NO API / NO ORDERS")
        print("=" * 118)
        for k, v in out.items():
            print(f"{k:60s}: {v}")
    if not ok:
        raise RuntimeError(f"Q50 M1-M12 V2.9.2 static self-check failed: {out}")
    return out
...
def start_q50(*, arm_phrase=None, runtime_hours=Q50_HOURS,
              max_start_loss_usd=Q50_MAX_LOSS_USD,
              min_start_equity_usd=Q50_MIN_EQUITY_USD):
    """REAL ORDERS: Q50 M1->M12, one hour, $20 software-loss trigger."""
    ...
    if abs(float(runtime_hours) - Q50_HOURS) > 1e-12:
        raise RuntimeError("Q50 M1-M12 V2.9.2 is fixed to exactly 1.0 hour.")
    if abs(float(max_start_loss_usd) - Q50_MAX_LOSS_USD) > 1e-12:
        raise RuntimeError("Q50 M1-M12 V2.9.2 is fixed to exactly a $20 software loss trigger.")
    if float(min_start_equity_usd) + 1e-12 < Q50_MIN_EQUITY_USD:
        raise RuntimeError("Q50 M1-M12 V2.9.2 requires minimum starting equity of at least $125.")
    ...
    mode = "DEEP_TAIL_Q50_M1_M12_1H_V18_V292"
    ...
    cfg = {
        ...
        "legacy_cleanup_event_names": "M5_* are inherited labels; actual boundary is M12/720s",
        "rest_fill_reconcile_mode": "MIN_TS_INCREMENTAL_DEDUP_CURSOR",
        "scientific_status": "FRESH_FORWARD_Q50_M1_M12_EXPLICIT_USER_REQUEST_NO_AUTOSCALE",
        "no_auto_scale": True,
    ...
    child = [
        sys.executable,
        "-m", "quant_research.kalshi.mm_deep_tail_join_ask_q50_m1_m12_live_v2_9_2",
    ...
    while time.time() < deadline:
        if p.poll() is not None:
            ...
            raise RuntimeError(
                f"Q50 M1-M12 V2.9.2 process exited during startup rc={p.returncode}\n{tail}"
            )

        last = B._read(session / "health.json", {}) or {}
        raw_health = B._read(session / "raw_capture" / "health.json", {}) or {}
        boundary = B._read(session / "m1_m12_boundary_spec.json", {}) or {}
        ...
        boundary_ok = (
            abs(float(boundary.get("entry_start_elapsed_s", -1.0)) - M1_S) < 1e-12
            and abs(float(boundary.get("terminal_cleanup_elapsed_s", -1.0)) - M12_S) < 1e-12
            and abs(float(boundary.get("recorder_end_elapsed_s", -1.0)) - M12_S) < 1e-12
        )
        ...
        if (
            state_ok and private_ok and raw_ok and recorder_ok and boundary_ok
        ...
        B._atomic(session / "KILL_REQUEST.json", {
            "time": B._iso(),
            "reason": "STARTUP_HEALTH_TIMEOUT_Q50_M1_M12_V292",
        })
        tail = log.read_text(encoding="utf-8", errors="replace")[-18000:] if log.exists() else ""
        raise RuntimeError(
            f"Q50 M1-M12 V2.9.2 startup health timeout. Last health={last}\n{tail}"
        )
    ...
    print("\n" + "=" * 118)
    print("REAL-MONEY DEEP-TAIL Q50 M1->M12 V2.9.2 ARMED")
    print("=" * 118)
    ...
    print("Strategy window:            M1 (60s) through M12 (720s)")
    print("Raw capture:                M0 through M12 + 30s label tail")
    print("Terminal cleanup:           M12 / 720s (legacy M5_* event labels)")
