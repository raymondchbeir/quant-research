...
"""V2.5 staged deployment for the V1.4 persistent-M5-cleanup engine.

V2.4 stopped the first Q5 run when a single M5 resting-order verification still
showed an order after the normal cancellation path. V1.4 changes that M5 condition
into a persistent per-ticker cleanup state: keep cancelling/verifying and keep
flattening residual inventory until the ticker is simultaneously flat and has zero
strategy resting orders. A transient M5 cleanup delay no longer triggers the whole
order group or ends the overnight run.

Q5 remains hard-gated behind a NEW completed Q1 smoke on this exact git HEAD and
V1.4 engine version. Importing this module sends no orders.
"""
...
from . import mm_deep_tail_join_ask_live_v1_4 as LIVE
...
DEPLOY_VERSION = "MM_DEEP_TAIL_JOIN_ASK_DEPLOY_V2_5_V1_4_PERSISTENT_M5_CLEANUP"
Q1_ARM = "LIVE_DEEP_TAIL_Q1_V14"
Q5_ARM = "LIVE_DEEP_TAIL_Q5_OVERNIGHT_V14"
KILL_ARM = B.KILL_ARM
...
PROMOTION_PATH = CORE.ROOT / "q1_operational_promotion_v2_5.json"
...
def static_self_check(*, show=True):
    ...
    checks = {
        ...
        "passive_join_ask_not_reduce_only": core.get("passive_join_ask_reduce_only") is False,
        "passive_join_ask_gtc": core.get("passive_join_ask_tif") == "good_till_canceled",
        "cleanup_reduce_only_ioc": (
            core.get("cleanup_reduce_only") is True
            and core.get("cleanup_tif") == "immediate_or_cancel"
        ),
        "persistent_m5_cleanup": core.get("m5_persistent_cleanup") is True,
        "no_immediate_m5_group_trigger": core.get("m5_immediate_group_trigger_on_resting") is False,
        "targeted_m5_recancel": core.get("m5_targeted_authoritative_recancel") is True,
        "wall_clock_m5_retry": core.get("m5_retry_on_wall_clock_loop") is True,
        "snapshot_m5_retry": core.get("m5_retry_on_post_m5_book_snapshot") is True,
        "m5_requires_zero_resting": core.get("m5_requires_exchange_zero_resting") is True,
        "m5_requires_flat": core.get("m5_requires_flat_position") is True,
        "no_passive_exit_after_m5": core.get("m5_passive_exit_suppressed_after_boundary") is True,
        "m5_auditor_handoff": core.get("m5_auditor_handoff_instead_of_orphan_kill") is True,
        "non_m5_hard_stops": core.get("non_m5_hard_stops_preserved") is True,
        "failed_create_track_cleanup": core.get("failed_create_local_track_retired_before_shutdown") is True,
        "retired_track_ghost_cancel": core.get("retired_track_accepted_create_cancelled") is True,
    ...
    if show:
        print("=" * 100)
        print("DEEP-TAIL DEPLOY V2.5 STATIC CHECK — NO ORDERS")
        print("=" * 100)
        for k, v in out.items():
            print(f"{k:50s}: {v}")
    if not ok:
        raise RuntimeError(f"V2.5 static self-check failed: {out}")
    return out
...
def _launch(*, q, hours, max_loss, min_equity, mode, arm_phrase, expected_arm):
    ...
    session = (CORE.ROOT / f"{stamp}_{mode.lower()}_deploy_v2_5").resolve()
    ...
    cfg = {
        ...
        "deploy_version": DEPLOY_VERSION,
        "scientific_status": "FRESH_FORWARD_LIVE_EXECUTION_VALIDATION_AFTER_PERSISTENT_M5_CLEANUP_FIX",
        "no_auto_scale": True,
    }
    ...
    child = [
        sys.executable,
        "-m", "quant_research.kalshi.mm_deep_tail_join_ask_deploy_v2_5",
    ...
    while time.time() < deadline:
        if p.poll() is not None:
            ...
            raise RuntimeError(f"Live V2.5 process exited during startup rc={p.returncode}\n{tail}")
        ...
        B._atomic(session / "KILL_REQUEST.json", {
            "time": B._iso(), "reason": "STARTUP_HEALTH_TIMEOUT_V2_5"
        })
        tail = log.read_text(encoding="utf-8", errors="replace")[-16000:] if log.exists() else ""
        raise RuntimeError(f"V2.5 startup health timeout. Last health={last}\n{tail}")

    print("\n" + "=" * 100)
    print("REAL-MONEY DEEP-TAIL V2.5 PROCESS ARMED")
    print("=" * 100)
    ...
    print("Passive fixed exit:        GTC, reduce_only=False")
    print("M5/risk cleanup:           IOC, reduce_only=True")
    print("Persistent M5 cleanup:     ENABLED")
    print("M5 targeted re-cancel:     ENABLED")
    print("M5 single-ticker delay:    DOES NOT STOP WHOLE RUN")
...
def q1_promotion_check(session_dir, *, show=True, write_receipt=True):
    ...
    if show:
        print("=" * 100)
        print("Q1 V1.4 -> Q5 OPERATIONAL PROMOTION — READ ONLY")
        print("=" * 100)
...
def _require_q1_promotion():
    ...
    if not r or r.get("passed") is not True:
        raise RuntimeError("Q5 HARD GATE: no passing V1.4 Q1 promotion receipt.")
...
def start_q5_overnight(*, arm_phrase=None,
    ...
    return _launch(
        ...
        max_loss=float(max_start_loss_usd),
        min_equity=float(min_start_equity_usd),
        mode="DEEP_TAIL_Q5_OVERNIGHT_V14",
        arm_phrase=arm_phrase,
        expected_arm=Q5_ARM,
    )
