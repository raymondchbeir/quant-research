...
"""V2.4 staged deployment for the V1.3 deep-tail engine.

This launcher supersedes V2.3 after the second real Q1 smoke proved that the live V2
API rejects reduce_only on a resting GTC JOIN_ASK exit.  V1.3 uses a normal passive
GTC exit for the fixed JOIN_ASK and preserves reduce-only IOC for M5/risk cleanup.

Q5 remains hard-gated behind a NEW completed Q1 smoke on this exact git HEAD and
V1.3 engine version.  Importing this module sends no orders.
"""
...
from . import mm_deep_tail_join_ask_live_v1_3 as LIVE
...
DEPLOY_VERSION = "MM_DEEP_TAIL_JOIN_ASK_DEPLOY_V2_4_V1_3_PASSIVE_GTC_COMPAT"
Q1_ARM = "LIVE_DEEP_TAIL_Q1_V13"
Q5_ARM = "LIVE_DEEP_TAIL_Q5_OVERNIGHT_V13"
KILL_ARM = B.KILL_ARM
...
PROMOTION_PATH = CORE.ROOT / "q1_operational_promotion_v2_4.json"
...
def static_self_check(*, show=True):
    ...
    checks = {
        ...
        "m5_cancel_confirm_before_flatten": core.get("m5_cancel_confirm_before_flatten") is True,
    ...
    if show:
        print("=" * 100)
        print("DEEP-TAIL DEPLOY V2.4 STATIC CHECK — NO ORDERS")
        print("=" * 100)
        for k, v in out.items():
            print(f"{k:48s}: {v}")
    if not ok:
        raise RuntimeError(f"V2.4 static self-check failed: {out}")
    return out
...
def _launch(*, q, hours, max_loss, min_equity, mode, arm_phrase, expected_arm):
    ...
    session = (CORE.ROOT / f"{stamp}_{mode.lower()}_deploy_v2_4").resolve()
    ...
    cfg = {
        ...
        "deploy_version": DEPLOY_VERSION,
        "scientific_status": "FRESH_FORWARD_LIVE_EXECUTION_VALIDATION_AFTER_PASSIVE_EXIT_API_FIX",
        "no_auto_scale": True,
    }
    ...
    child = [
        sys.executable,
        "-m", "quant_research.kalshi.mm_deep_tail_join_ask_deploy_v2_4",
    ...
    while time.time() < deadline:
        if p.poll() is not None:
            ...
            raise RuntimeError(f"Live V2.4 process exited during startup rc={p.returncode}\n{tail}")
        ...
        B._atomic(session / "KILL_REQUEST.json", {
            "time": B._iso(), "reason": "STARTUP_HEALTH_TIMEOUT_V2_4"
        })
        tail = log.read_text(encoding="utf-8", errors="replace")[-16000:] if log.exists() else ""
        raise RuntimeError(f"V2.4 startup health timeout. Last health={last}\n{tail}")

    print("\n" + "=" * 100)
    print("REAL-MONEY DEEP-TAIL V2.4 PROCESS ARMED")
    print("=" * 100)
    ...
    print("Failed-create cleanup:     ENABLED")
...
def q1_promotion_check(session_dir, *, show=True, write_receipt=True):
    ...
    if show:
        print("=" * 100)
        print("Q1 V1.3 -> Q5 OPERATIONAL PROMOTION — READ ONLY")
        print("=" * 100)
...
def _require_q1_promotion():
    ...
    if not r or r.get("passed") is not True:
        raise RuntimeError("Q5 HARD GATE: no passing V1.3 Q1 promotion receipt.")
...
def start_q5_overnight(*, arm_phrase=None,
    ...
    return _launch(
        ...
        max_loss=float(max_start_loss_usd),
        min_equity=float(min_start_equity_usd),
        mode="DEEP_TAIL_Q5_OVERNIGHT_V13",
        arm_phrase=arm_phrase,
        expected_arm=Q5_ARM,
    )
