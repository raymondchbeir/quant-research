...
"""Dashboard-compatible V3.0.0 deployment shim with Deep35 audit reconciliation.

The public V3.0.0 / V1.13 version strings are intentionally preserved so the
existing notebook dashboard remains compatible without modification.  The exact
source commit plus PATCH_VERSION distinguish this fixed deployment from the prior
defective run.
...
from . import mm_deep_tail_join_ask_live_v1_13_1_audit_reconcile as LIVE


DEPLOY_VERSION = V300.DEPLOY_VERSION
PATCH_VERSION = "V3_0_0_AUDITFIX1"
MODULE_NAME = (
    "quant_research.kalshi."
    "mm_deep_tail_join_ask_q50_deep35_hyst5_rec10_live_v3_0_0_auditfix1"
)

Q50_ARM = "LIVE_DEEP35_HYST5_Q50_REC10_12H_V300_AUDITFIX1"
...
SUPERVISOR_LOG_FILE = "supervisor_v3_0_0_deep35_auditfix1.log"
GUARDIAN_LOG_FILE = "guardian_v3_0_0_deep35_auditfix1.log"
STARTUP_TIMEOUT_S = V300.STARTUP_TIMEOUT_S


def _install_patch():
    """Rebind the audited V3.0.0 operational wrapper to the fixed child engine."""
    # V300 functions resolve these globals dynamically, so rebinding them is enough
    # to preserve its recorder/supervisor/guardian/shard/preflight implementation.
    V300.LIVE = LIVE
    ...
    # Ensure detached runtime launches this shim, not the historical wrapper path.
...
def static_self_check(*, show=True):
    ...
    checks = {
        ...
        "auditfix_live_static_ok": live.get("ok") is True,
        ...
        "deploy_patch_version_exact": PATCH_VERSION == "V3_0_0_AUDITFIX1",
        "engine_patch_version_exact": getattr(LIVE, "PATCH_VERSION", None)
        == "DEEP35_AUDIT_RECONCILE_V1_13_1",
        "detached_module_is_auditfix": RUNTIME.MODULE_NAME == MODULE_NAME,
        "runtime_live_is_auditfix": RUNTIME.LIVE is LIVE,
        "parent_live_is_auditfix": P.LIVE is LIVE,
        "guardian_live_is_auditfix": V2963.LIVE is LIVE,
    ...
    if show:
        print("=" * 168)
        print("V3.0.0 DEEP35 AUDITFIX1 STATIC CHECK — NO API / NO ORDERS")
        print("=" * 168)
        for k, v in out.items():
            print(f"{k:100s}: {v}")
    if not ok:
        raise RuntimeError(f"V3.0.0 AUDITFIX1 static self-check failed: {out}")
    return out
...
def live_status(*, show=True, tail_lines=40):
    _install_patch()
    out = V300.live_status(show=show, tail_lines=tail_lines)
    return out
