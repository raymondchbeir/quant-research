...
"""Fixed 12-hour Q50 overnight launcher using the V1.9 stale-orphan guard.

Operational wrapper only. Strategy/execution rules are unchanged from V2.9.3:
- Q50;
- M1 dual 5c entries;
- first-fill-wins and opposite-tail cancellation;
- selected tail accumulates only until M5;
- fixed passive JOIN_ASK/no repricing after a full Q50 entry;
- persistent verified M5 cleanup;
- no strategy orders after M5;
- raw M0->M12 capture plus 30-second label tail;
- V1.9 full-terminal user_order tombstones;
- V1.7 bounded-memory/incremental fill reconciliation;
- 2 GiB guardian;
- no autoscaling.

Only the requested runtime changes from 1 hour to exactly 12 hours.
The $20 equity-loss threshold is a SOFTWARE STOP, not a guaranteed maximum
realized loss. Importing this module sends no API requests and no orders.
"""
...
DEPLOY_VERSION = "MM_DEEP_TAIL_JOIN_ASK_Q50_M1_M5_RECORD_M12_V2_9_4_OVERNIGHT_12H"
CORE = V282.CORE
Q50_ARM = "LIVE_DEEP_TAIL_Q50_M1_M5_RECORD_M12_12H_V294"
...
def static_self_check(*, show=True):
    ...
    checks = {
        ...
        "strategy_rules_unchanged_from_v293": True,
    }
    ...
    if show:
        print("=" * 124)
        print("Q50 M1->M5 + RECORD M12 V2.9.4 OVERNIGHT 12H STATIC CHECK — NO API / NO ORDERS")
        print("=" * 124)
        for k, v in out.items():
            print(f"{k:66s}: {v}")
    if not ok:
        raise RuntimeError(f"Q50 overnight V2.9.4 static check failed: {out}")
    return out
...
def start_q50(*, arm_phrase=None, runtime_hours=Q50_HOURS,
    ...
    if abs(float(runtime_hours) - Q50_HOURS) > 1e-12:
        raise RuntimeError("Q50 V2.9.4 overnight is fixed to exactly 12.0 hours.")
    if abs(float(max_start_loss_usd) - Q50_MAX_LOSS_USD) > 1e-12:
        raise RuntimeError(
            "Q50 V2.9.4 overnight is fixed to exactly a $20 software loss trigger."
        )
    if float(min_start_equity_usd) + 1e-12 < Q50_MIN_EQUITY_USD:
        raise RuntimeError(
            "Q50 V2.9.4 overnight requires minimum starting equity of at least $125."
        )
    ...
    mode = "DEEP_TAIL_Q50_M1_M5_RECORD_M12_12H_V19_V294"
    ...
    cfg = {
        ...
        "scientific_status": "Q50_M1_M5_12H_FORWARD_WITH_M12_RECORDING_V19",
        "no_auto_scale": True,
    ...
    child = [
        sys.executable,
        "-m", "quant_research.kalshi.mm_deep_tail_join_ask_q50_record_m12_live_v2_9_4_overnight",
    ...
    while time.time() < deadline:
        if proc.poll() is not None:
            ...
            raise RuntimeError(
                f"Q50 V2.9.4 overnight process exited during startup rc={proc.returncode}\n{tail}"
            )
        ...
        B._atomic(session / "KILL_REQUEST.json", {
            "time": B._iso(),
            "reason": "STARTUP_HEALTH_TIMEOUT_Q50_V294_OVERNIGHT",
        })
        tail = log.read_text(encoding="utf-8", errors="replace")[-18000:] if log.exists() else ""
        raise RuntimeError(
            f"Q50 V2.9.4 overnight startup health timeout. Last health={last}\n{tail}"
        )
    ...
    print("\n" + "=" * 124)
    print("REAL-MONEY Q50 M1->M5 + M12 RECORDING V2.9.4 OVERNIGHT ARMED")
    print("=" * 124)
