...
"""V2.9.9.2 real-money Q50 / 12h REC25 deployment.

Operational baseline is V2.9.9.1.  This wrapper swaps only the live engine to
V1.12.5, which preserves V1.12.4's atomic REC25 trigger/price, exact Kalshi
equity, and hardened M12 cleanup while fixing the current Kalshi API constraint
that ``reduce_only`` is accepted only for IOC orders.

The fixed REC25 exit remains passive, post-only, good-till-canceled, and never
reprices.  Therefore V1.12.5 sends that passive EXIT with reduce_only=False.
Risk/M12 flattening remains on the inherited reduce-only IOC path.

Entry, Q50, M1, M12 danger guard, REC25 anchor/threshold, 12-hour runtime,
$20 software loss trigger, $125 minimum equity, recorder, generation rotation,
stale-REST reconciliation, and orphan fail-closed behavior remain unchanged.

Importing this module performs no API calls and sends no orders.
"""

from . import mm_deep_tail_join_ask_q50_m12_guard_rec25_live_v2_9_9_1_12h_rotation as BASE
from . import mm_deep_tail_join_ask_live_v1_12_5_rec25_passive_exit_compatible as REC25


DEPLOY_VERSION = "MM_DEEP_TAIL_JOIN_ASK_Q50_M1_M12_GUARD_REC25_V2_9_9_2_12H_ROTATION"
MODULE_NAME = "quant_research.kalshi.mm_deep_tail_join_ask_q50_m12_guard_rec25_live_v2_9_9_2_12h_rotation"
Q50_ARM = "LIVE_DEEP_TAIL_Q50_M1_M12_GUARD_REC25_12H_V2992"
...
def _install_patch():
    """Install V2.9.9.1 operations, then replace LIVE with V1.12.5."""
    BASE._install_patch()
...
def static_self_check(*, show=True):
    ...
    checks = {
        "base_v2991_ok": base.get("ok") is True,
        "rec25_v1_12_5_ok": rec.get("ok") is True,
        "runtime_live_is_v1_12_5": RUNTIME.LIVE is REC25,
        "parent_live_is_v1_12_5": P.LIVE is REC25,
        "guardian_live_is_v1_12_5": V2963.LIVE is REC25,
        "rec25_engine_exact": LIVE.M12GuardRotatingGenerationEngine is LIVE.Rec25PassiveExitM12Engine,
        ...
        "detached_module_points_to_v2992": RUNTIME.MODULE_NAME == MODULE_NAME,
    ...
    out = {
        ...
        "rec25": {
            ...
            "trigger_price_source": "SAME_CERTIFIED_BBO_AS_TRIGGER",
            "passive_exit_reduce_only": False,
            "passive_exit_post_only": True,
            "passive_exit_tif": LIVE.PASSIVE_EXIT_TIF,
        },
        **checks,
        "ok": bool(ok),
    }
    ...
    if show:
        print("=" * 160)
        print("V2.9.9.2 Q50 M1->M12 GUARD + REC25 PASSIVE-EXIT STATIC CHECK — NO API / NO ORDERS")
        print("=" * 160)
        for k, v in out.items():
            print(f"{k:96s}: {v}")

    if not ok:
        raise RuntimeError(f"V2.9.9.2 static self-check failed: {out}")
    return out


def q50_preflight(*, show=True):
    """Read-only preflight using exact-dollar equity semantics."""
    static_self_check(show=show)
    V28._patch_parent()
    V28.D._guard_other_live_processes()

    old_equity = REC25.V1.B._equity
    REC25.V1.B._equity = REC25.exact_equity_from_balance
    try:
        ...
        REC25.V1.B._equity = old_equity


def start_q50_12h_smoke(*, arm_phrase=None):
    """REAL-MONEY Q50 / 12h REC25 V1.12.5 entrypoint."""
    _install_patch()
